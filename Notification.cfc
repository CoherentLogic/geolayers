component accessors="true" displayname="Notification" {
    ...
    public Notification function send(required array users) output=true
    {
        ...
        for(user in users) {
            if(mumps.lock("geodigraph", ["accounts", user.email, "notifyIndex"], 10)) {
                ...
                module template="/v2/modules/send_email.cfm" caption=this.caption message=this.message link=this.link icon=this.icon recipient=user.email;
